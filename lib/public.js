 ...
 */
export async function fetchFrom ({ client, token, cid, head = false, timeoutMs = 8000 }) {
  const id = `f${++rid}`
  ...
  client.send(token, { type: MSG.FETCH, rid: id, cid, head: !!head })
  const p = await reply
  let bytes = null
 ...
 *
 * @param {{ client: any, owner: string, cid: string, head?: boolean, timeoutMs?: number }} opts
 *   head: solo metadatos y URL, sin bytes (para decidir si cargar por URL).
 * @throws {Error & {code:'no-node'|'not-found'|'private'|'too-large'|'corrupt'|'timeout'}}
 */
export async function fetchPublic ({ client, owner, cid, head = false, timeoutMs = 8000 }) {
  ...
  for (const token of tokens) {
    try {
      return await fetchFrom({ client, token, cid, head, timeoutMs })
    } catch (e) {
      last = e
